...
// DMH: This member function performs the operation vec_defl = U (\Sigma)^(-1) V^dag vec_in
//      where U and V are the Left and Right singular vectors of the unpreconditioned 
//      operator M. We calculate the Right Singular Vectors of M via the exact deflation
//      routines,
// 
//      MdagM v = \lambda v  (where \lambda = \sigma^2)
//
//      from which we can derive U via,
//
//      M = U \Sigma Vdag <=> U = (M V \Sigma^{-1})
//
//      hence the deflation operator is,
//     
//      vev_defl = (M V \Sigma^{-2}) Vdag) vec_in
//
//      This has exactly the same form as deflateVector, expect that a single RHS with 
//      M must be applied after the V\Sigma^{-2}Vdag operator is complete.

/*
template <typename Float>
void QKXTM_Deflation<Float>::
deflateVectorSVD(QKXTM_Vector<Float> &vec_defl, 
		 QKXTM_Vector<Float> &vec_in){
  if(NeV == 0){
    vec_defl.zero_device();
    return;
