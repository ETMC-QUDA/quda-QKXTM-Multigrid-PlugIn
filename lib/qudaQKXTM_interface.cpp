  ...
  for(int isource = 0 ; isource < info.Nsources ; isource++){
	...
	*( (double*)input_vector + 
	...
	K_vector->scaleVector(2*param->kappa);
      ...
      //CJL: debug
      K_prop_up->absorbVectorToHost(*K_temp,isc/3,isc%3);
	...
	*( (double*)input_vector + 
	...
	K_vector->scaleVector(2*param->kappa);
      ...
      //CJL: debug
      K_prop_down->absorbVectorToHost(*K_temp,isc/3,isc%3);
    ...
    asprintf(&filename_threep_base,"%s_nucleon_src%d_up_db.dat",
	     filename_threep, isource);
    K_prop_up->write_ASCII(filename_threep_base);

    asprintf(&filename_threep_base,"%s_nucleon_src%d_down_db.dat",
	     filename_threep, isource);
    K_prop_down->write_ASCII(filename_threep_base);
...
void calcMG_threepTwop_Mesons_db(void **gauge_APE, void **gauge, 
			       QudaGaugeParam *gauge_param, 
			       QudaInvertParam *param, 
			       qudaQKXTMinfo info, 
			       char *filename_twop, 
  ...
  double tx3,tx4,summ_tx34=0.; // needed to time just the inversion time
  char fname[256];
  sprintf(fname, "calcMG_threepTwop_Meson_db");
  ...
  for(int isource = 0 ; isource < info.Nsources ; isource++){
	...
	*( (double*)input_vector + 
    ...
    // CJL: write propagators for debug

    char *filename_threep_base;
    asprintf(&filename_threep_base,"%s_meson_src%d_up_db.dat",
	     filename_threep, isource);
    K_prop_up->write_ASCII(filename_threep_base);

    asprintf(&filename_threep_base,"%s_meson_src%d_down_db.dat",
	     filename_threep, isource);
    K_prop_down->write_ASCII(filename_threep_base);
