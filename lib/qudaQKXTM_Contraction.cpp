...
void QKXTM_Contraction<Float>::writeThrpHDF5_MomSpace_HighMomForm(void *Thrp_local_HDF5, void *Thrp_noether_HDF5, void **Thrp_oneD_HDF5, char *filename, qudaQKXTMinfo info, int isource, WHICHPARTICLE HADRON){
  ...
  if(GK_timeRank >= 0 && GK_timeRank < GK_nProc[3] ){
	...
	for(int part=0;part<2;part++){
      ...
      else errorQuda("Error (writeThrpHDF5_MomSpace_HighMomForm): Invalid hadron"); 
      H5Gclose(group3_id);
    }//-its
...
writeThrp_ASCII(void *corrThp_local, 
  ...
  else if(testParticle == KAON){
    strcpy(fname_particle,"kaon");
    if(partflag == 1)strcpy(fname_upORdown,"up");
    else if(partflag == 2)strcpy(fname_upORdown,"strange");
    else errorQuda("writeThrp_ASCII: Got the wrong part! Should be either 1 or 2.");
  }
